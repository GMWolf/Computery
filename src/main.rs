...
use wgpu::{ShaderStage, BindGroupLayoutEntry, StorageTextureAccess, TextureFormat, Device};
...
fn compile_shader(source: &str, shader_kind: shaderc::ShaderKind, filename: &str, entry: &str) -> Vec<u32> {
    ...
    options.set_binding_base(shaderc::ResourceKind::StorageBuffer, 16);
...
fn lua_buffer(ctx: &mut ScriptContext, def: &rlua::Table) -> usize {
    let size = def.get("size").unwrap();

    ctx.resources.push(ScriptResources::Buffer(
        ScriptBuffer{
            size,
        }
    ));
...
fn lua_pass_internal(def: &rlua::Table) -> ScriptPass {
    ...
    let binding_table: rlua::Table = def.get("bindings").unwrap();
    for a in binding_table.pairs() {
        let (name, binding) = a.unwrap();
        bindings.insert(name, binding);
    };

    ScriptPass { shader, entry, bindings, x_threads, y_threads, z_threads }
}

fn lua_pass(ctx: &mut ScriptContext, def: &rlua::Table) {
    ctx.passes.push(lua_pass_internal(def));
}

fn lua_init_pass(ctx: &mut ScriptContext, def: &rlua::Table) {
    ctx.init_passes.push(lua_pass_internal(def));
}
    ...
    lua.context(|lua_ctx| {
        ...
        lua_ctx.scope(|scope| {
            ...
            globals.set(
                ...
                scope.create_function_mut(|_, table: rlua::Table| {
                    let mut ctx = scriptCtx.borrow_mut();
                    Ok(lua_init_pass(ctx.deref_mut(), &table))
                })?,
            )?;
            ...
            globals.set(
                ...
                scope.create_function_mut(|_, table: rlua::Table| {
                    let mut ctx = scriptCtx.borrow_mut();
                    ctx.display = Some(table.get("image").unwrap());
                    Ok(())
                })?,
            )?;
            ...
            globals.set(
                ...
                scope.create_function_mut(|_, table: rlua::Table| {
                    let mut ctx = scriptCtx.borrow_mut();
                    Ok(lua_buffer(ctx.deref_mut(), &table))
                })?,
            )?;
...
fn run(event_loop: EventLoop<()>, window: Window, ctx: &ScriptContext) {
    ...
    let resources: Vec<_> = ctx.resources.iter().map(|res| {
        match res {
            ScriptResources::Texture(tex_def) => {
                ...
                Resource::Texture(tex, view)
            }

            ScriptResources::Buffer(buf_def) => {
    ...
    let shaders: Vec<_> = ctx.passes.iter().chain(ctx.init_passes.iter())
        .map(|pass_def| (&pass_def.shader, &pass_def.entry)).unique()
        .map(|(file, entry)| {
    ...
    let passes: Vec<_> = ctx.passes.iter().map(|pass_def| {
        create_pass(&device, &resources, &shaders, pass_def)
    }).collect();

    let init_passes: Vec<_> = ctx.init_passes.iter().map(|pass_def| {
        create_pass(&device, &resources, &shaders, pass_def)
    }).collect();

    let display_view = match &resources[ctx.display.unwrap()] {
...
fn create_pass(device: &Device, resources: &Vec<Resource>, shaders: &Vec<Shader>, pass_def: &ScriptPass) -> Pass {
    ...
    let entries: Vec<_> = pass_def.bindings.iter().map(|(name, &resource_id)| {
        wgpu::BindGroupEntry {
            ...
            resource: match &resources[resource_id] {
                Resource::Texture(_, view) => {
...
fn main() {
    ...
    let lua = read_to_string(&args[1]).unwrap();
    let ctx = parse_lua(&lua).unwrap();
    run(event_loop, window, &ctx);
}
